...
function submitTaskForm(event, newTaskOrEditTask) {
  ...
  if (newTaskOrEditTask == "newTask") {
    if (
      masterTaskList.some(
        (taskObject) =>
          taskObject.taskID == event.target.elements[0].value + myProjectPage
      )
    ) {
      alert(
    ...
    if (masterTaskList[editedIndex].dueDate != "") {
      masterTaskList[editedIndex].dueDate = format(
        new Date(masterTaskList[editedIndex].dueDate + "T00:00"),
        "MM/dd/yyyy"
      );
    }
    masterTaskList[editedIndex].taskID =
      masterTaskList[editedIndex].name + masterTaskList[editedIndex].project;
  }
  closeTaskForm();
...
taskForm.addEventListener("submit", function () {
  submitTaskForm(event, newTaskOrEditTask);
});
